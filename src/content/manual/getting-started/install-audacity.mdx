...
<Tabs>
  ...
  <Tab label="macOS">
    Download the `.dmg` disk image, then open it and drag the Audacity icon
...
<BestPractices>
  - Download from audacityteam.org rather than a third-party mirror — builds
    elsewhere are sometimes bundled with software you didn't ask for.
  - Keep the installer file until you've confirmed Audacity opens, in case you
    need to run it again.
</BestPractices>

<Pitfalls>
  - Installing over a much older version can carry settings forward that no
    longer apply. If Audacity behaves oddly after an upgrade, resetting
    preferences is usually the fastest fix.
  - On managed machines, an install that appears to succeed but leaves no
    application behind usually means it was blocked by policy.
</Pitfalls>
