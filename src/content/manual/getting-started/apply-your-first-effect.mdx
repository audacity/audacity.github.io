...
Each effect in the stack is managed from the panel:

- **Change its settings** by clicking the effect's name to reopen its
  window. Effect windows often have their own look, nothing like Audacity:
  that is normal, and the main window stays usable while one is open.
- **Bypass** an effect with the power button beside it, or the whole stack
  with the power button at the top. This is the quickest way to hear
  before-and-after.
...
<Callout type="tip" title="Start with realtime effects">
  ...
  change and undo your thinking without changing the audio data. Use the Effect
  menu when you know what you want.
</Callout>

## Master effects

At the bottom of the same panel is a **Master effects** section. Effects
here apply to the finished mix, after every track, each track's own effects,
and the volume and pan controls have been combined, rather than to any one
track. This stage of shaping the combined sound is what "mastering" means.

<UIShot
  name="app/SceneEffectsPanel"
  state="master"
  alt="The Master effects section at the bottom of the panel, with a Limiter and Reverb stacked on the mix"
/>
