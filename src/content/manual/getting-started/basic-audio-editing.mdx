...
Almost every edit in Audacity is the same two moves: choose the audio you
want to change, then run a command on it. Every editing command is a
variation on that loop, and nothing in it is permanent.
...
<UIShot
  ...
  alt="An unselected clip, with its name along the top and the waveform inside"
/>
...
<UIShot
  name="trackpanel/WaveClip"
  state="selected"
  alt="A selected clip, showing the trim and speed handles at its edges"
/>
...
<Shortcut client:load keys="cmd" /> and scroll to zoom around the pointer, then
adjust the edges of the selection at the finer scale.
...
<Shortcut client:load keys="cmd+shift+z" />, steps forward again if you went too
far.
