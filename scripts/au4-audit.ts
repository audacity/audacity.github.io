...
const TOPICS_PATH = join(ROOT, "scripts/au3-topic-checklist.json");
...
type MapEntry = {
  ...
  status?: string;
  reason?: string;
  /** For status "external": where the coverage lives (a non-collection page). */
  note?: string;
};
...
type Coverage =
  ...
  /* Covered by something outside the content collection (a generated page
     or site route) that the manual walk can't see. */
...
function resolveCoverage(
  ...
  if (entry?.status === "external") {
    return { status: "external", note: entry.note ?? "" };
  }
  if (entry?.page) {
...
function statusCell(c: Coverage): string {
  switch (c.status) {
    ...
    case "external":
      return "✅ written (outside collection)";
  }
}

function pageCell(c: Coverage): string {
  if (c.status === "external") return c.note || "—";
...
function tally(coverages: Coverage[]): Record<string, number> {
  ...
  // External coverage is written coverage that lives outside the collection.
  for (const c of coverages)
    t[c.status === "external" ? "written" : c.status]++;
  return t;
}
...
async function main() {
  ...
  const checklist: { topics: Topic[] } = existsSync(TOPICS_PATH)
    ? JSON.parse(await readFile(TOPICS_PATH, "utf8"))
    : { topics: [] };
  const topicCoverage = checklist.topics.map((topic) => {
    // A topic is satisfied by its own title or any listed alias — take the
    // best coverage across them (map override on topic:<id> wins as usual).
    let best: Coverage = resolveCoverage(
      `topic:${topic.id}`,
      topic.title,
      map,
      pagesBySlug,
    ...
    for (const alias of topic.match ?? []) {
      if (best.status === "written") break;
      const alt = resolveCoverage(
        `topic:${topic.id}`,
        alias,
        map,
        pagesBySlug,
  ...
  for (const { topic, coverage } of topicCoverage) {
    if (coverage.status === "missing" && topic.status === "applies") {
      nextPages.push(
        `topic **${topic.title}** (${topic.stream}, target-state checklist)`,
      );
    }
  }
  ...
  const topicTally = tally(topicCoverage.map((c) => c.coverage));
  ...
  row("Target-state topics (AU3 manual)", checklist.topics.length, topicTally);
  out(
  ...
  out("## Target-state topics (from the AU3 manual)");
  out();
  out("The topics manual.audacityteam.org covers, mapped to Audacity 4 —");
  out("the breadth a complete manual needs beyond the app's own surface.");
  out("Curated in scripts/au3-topic-checklist.json; “verify” means it is");
  out("not yet known whether AU4 has the feature at all.");
  out();
  out("| Topic | Stream | AU4 status | Coverage | Page |");
  out("| --- | --- | --- | --- | --- |");
  for (const { topic, coverage } of topicCoverage) {
    out(
      `| ${topic.title} | ${topic.stream} | ${topic.status} | ${statusCell(coverage)} | ${pageCell(coverage)} |`,
    );
  }
  out();
